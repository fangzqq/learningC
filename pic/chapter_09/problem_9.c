...
bool replaceString(char source[], char text[], char r[])
{
    int i, index, length;
...
int main (void)
{
    char source[100] = "abc defghi ";
    char text[] = "one";
    bool replaceString(char s1[], char s2[], char s3[]);
    bool stillFound;

    do
        stillFound = replaceString(source, "1", text);
    while(stillFound);
